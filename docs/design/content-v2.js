// ============================================================
// THE WORKSPACE v2 — all copy & content. Swap placeholders here.
...
export const PH = {
  ...
  BOOKING: "#",                    // [BOOKING LINK] — swap for Cal/Calendly URL
  STATUS_LINE: "Multiple multi-million valuations · 0 abandoned codebases",
  FOOTER: "⌘K — jump to any project or component",
};

export const KINDS = ["All", "Auth", "Discovery", "Dashboards", "Commerce", "Onboarding", "Admin"];

// ---- Component fragments (the bits & pieces) ----------------
export const FRAGMENTS = {
  multiverse: {
    title: "The living timeline", kind: "Discovery", product: "Paradox", year: "2026", prod: false, accent: "#3edba6", w: 560,
    call: "Every storytelling platform defaults to feeds and thumbnail grids. The call here: delete the grid. The timeline graph IS the interface — you drag through the multiverse, zoom into a strand, click a node to enter a reality. Navigation chrome shrinks to a single search box, because the structure is the map.",
    callLayer: "Interaction",
    code: { file: "MultiverseTimeline.tsx", lang: "tsx", src:
`// The homepage is the graph itself — no feed, no grid.
export function MultiverseTimeline({ graph }: { graph: Graph }) {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "The void — strands, glow, and nothing else" },
      { label: "Nodes & branches", sub: "Timelines fork into realities; gold marks variants" },
      { label: "Interaction", sub: "Drag to traverse, click a node to enter" },
      { label: "The graph", sub: "Timelines → branches → events, one data model" },
    ],
  },
  gateway: {
    title: "Sign-in gateway", kind: "Auth", product: "Paradox", year: "2026", prod: false, accent: "#3edba6", w: 360,
    call: "A story multiverse lives or dies on lurkers becoming writers, so the door stays open: guest first, magic link second, passwords never. Commitment is only requested the moment you try to branch a reality — by then the product has already made its argument.",
    callLayer: "Guest path",
    code: { file: "SignInGateway.tsx", lang: "tsx", src:
`// Guest first. Magic link second. Passwords never.
export function SignInGateway() {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "One door, three ways through it" },
      { label: "Providers & magic link", sub: "Google, Discord, or a one-time link" },
      { label: "Identity forge", sub: "A forged keeper name — no real names required" },
      { label: "Guest path", sub: "Explore everything, commit only to create" },
    ],
  },
  palette: {
    title: "Command palette", kind: "Discovery", product: "Paradox", year: "2026", prod: false, accent: "#3edba6", w: 430,
    call: "With no feed and no menus, one ⌘K palette carries the entire navigation: timelines, branches and events in a single index, grouped by what kind of thing you found. The discipline this buys: every screen stays a place, not a junction box of links.",
    callLayer: "The index",
    code: { file: "CommandPalette.tsx", lang: "tsx", src:
`// One Cmd-K index carries the whole navigation.
export function CommandPalette({ index }: { index: Entry[] }) {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "One field, summoned from anywhere" },
      { label: "Results & grouping", sub: "Timelines, branches, events — labelled, ranked" },
      { label: "Keyboard loop", sub: "↑ ↓ ↵ esc — hands never leave the keys" },
      { label: "The index", sub: "One searchable graph of the whole multiverse" },
    ],
  },
  onboarding: {
    title: "Onboarding", kind: "Onboarding", product: "Shiftsum", year: "2022", prod: false, accent: "#e3a84e", w: 400,
    call: "Nine required fields became three questions with smart defaults — people change defaults later, when they actually care. Activation went up 34% and setup-related support more or less disappeared.",
    callLayer: "Defaults engine",
    code: { file: "Onboarding.tsx", lang: "tsx", src:
`// Nine fields became three questions with smart defaults.
...
export function Onboarding() {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "Three questions, twenty seconds" },
      { label: "Steps & inputs", sub: "Big targets, obvious progress, no dead ends" },
      { label: "Defaults engine", sub: "Guesses well so users don't have to decide" },
      { label: "Team model", sub: "Where invites, roles and shifts actually live" },
    ],
  },
  notifications: {
    title: "Notification preferences", kind: "Admin", product: "Shiftsum", year: "2022", prod: false, accent: "#e3a84e", w: 380,
    call: "Defaults are the product. Quiet hours on by default, digests instead of pings, loud only for “your shift changed.” Unsubscribe rate stayed under 1% — in a product whose entire job is interrupting people.",
    callLayer: "Preference logic",
    code: { file: "NotificationPrefs.tsx", lang: "tsx", src:
`// Defaults are the product: quiet by default, loud only when it matters.
const DEFAULTS = {
...
export function NotificationPrefs() {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "Plain switches, no settings maze" },
      { label: "Switches & schedule", sub: "Every control reachable with a thumb" },
      { label: "Preference logic", sub: "Per-channel, per-event, time-zone aware" },
      { label: "Delivery", sub: "Push, SMS, email — each with a fallback" },
    ],
  },
  admin: {
    title: "Roster admin", kind: "Admin", product: "Shiftsum", year: "2023", prod: false, accent: "#e3a84e", w: 560,
    call: "The least glamorous screen in this portfolio and the one users thanked us for most. Instant search, filters, bulk approve, zero page reloads. Managers went from ~20 minutes a day in here to ~4.",
    callLayer: "Permissions",
    code: { file: "RosterAdmin.tsx", lang: "tsx", src:
`// Instant, client-side, forgives typos. ~20 min/day became ~4.
export function RosterAdmin({ people }: { people: Person[] }) {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "A table that respects your time" },
      { label: "Table & actions", sub: "Bulk approve, keyboard friendly" },
      { label: "Filtering & search", sub: "Instant, client-side, forgives typos" },
      { label: "Permissions", sub: "Who can approve what — the real complexity" },
    ],
  },
  booking: {
    title: "Booking widget", kind: "Commerce", product: "Client: physio clinic", year: "2024", prod: true, accent: "#a98bf5", w: 360,
    call: "A widget, not an app — it lives inside the clinic's existing site. No login, three taps to book. 41% of their bookings now arrive after office hours, when the front desk is asleep.",
    callLayer: "Availability rules",
    code: { file: "BookingWidget.tsx", lang: "tsx", src:
`// A widget, not an app — it drops into the clinic's existing site.
export function BookingWidget({ clinicId }: { clinicId: string }) {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "Three taps: day, time, done" },
      { label: "Slots & picker", sub: "Thumb-sized, works inside any page" },
      { label: "Availability rules", sub: "Buffers, breaks, double-booking guards" },
      { label: "Calendar sync", sub: "Two-way with the clinic's real calendar" },
    ],
  },
  kyc: {
    title: "Identity verification", kind: "Onboarding", product: "Client: fintech", year: "2025", prod: true, accent: "#a98bf5", w: 380,
    call: "The senior move here was NOT building it. I recommended a verification vendor and built the glue around it — upload UX, retries, status webhooks. Saved roughly 4 months and a compliance headache. Knowing what not to build is the job.",
    callLayer: "Vendor glue",
    code: { file: "IdentityCheck.tsx", lang: "tsx", src:
`// The senior move was NOT building this — just the glue around a vendor.
...
export function IdentityCheck() {
  ...
  return STEPS.map((s) => (
    ...
    layers: [
      { label: "Surface", sub: "Calm steps for a nervous moment" },
      { label: "Upload & camera", sub: "Clear states: sending, checking, done" },
      { label: "Vendor glue", sub: "Retries, timeouts, status webhooks" },
      { label: "Audit trail", sub: "Compliance wants receipts. It gets receipts." },
    ],
  },
  status: {
    title: "Public status page", kind: "Dashboards", product: "Client: SaaS API", year: "2025", prod: true, accent: "#a98bf5", w: 480,
    call: "A two-day build. The week it shipped, the client's API had a bad day — and support tickets halved, because customers could watch the fix happen instead of emailing about it. Cheapest trust money can buy.",
    callLayer: "Incident log",
    code: { file: "StatusPage.tsx", lang: "tsx", src:
`// Honest when it isn't green. A two-day build that halved support tickets.
export function StatusPage({ services }: { services: Service[] }) {
  ...
  return (
    ...
    layers: [
      { label: "Surface", sub: "Green when green, honest when not" },
      { label: "Uptime bars", sub: "90 days at a glance, no chart library" },
      { label: "Health checks", sub: "Pings every 60s from three regions" },
      { label: "Incident log", sub: "Written by humans — honesty included" },
    ],
  },
};
...
// ---- Shared library item -------------------------------------
const LIBRARY = {
  label: "Component library", type: "library",
...
export const WORKSPACES = [
  {
    id: "home", initials: "NH", name: PH.NAME, desc: "The person behind the products",
    status: "a living showcase", accent: "#5b8def",
    tags: [],
    groups: [
    ...
    items: {
      about: {
        label: "About", type: "page", desc: "Who you'd be working with",
        sections: [
          { paras: [
          ...
          { heading: "What I'm not", paras: [
            "An agency, a dev shop, or a body-leasing arrangement. You get me, my judgment, my agents, and the receipts.",
          ]},
          { heading: "If you're building something", cta: "message", ctaLabel: "Leave a message →", paras: [
      ...
      "how-i-work": {
        label: "How I work", type: "page", desc: "What an engagement actually looks like",
        sections: [
          { heading: "It starts with a conversation", paras: [
          ...
          { heading: "I mold to you and your team", paras: [
          ...
          { heading: "Results in days, not months", paras: [
          ...
          { heading: "I push the code to the edge", paras: [
      ...
      "straight-answers": {
        label: "Straight answers", type: "page", desc: "The honest FAQ",
        sections: [
          { heading: "“Should I just build it myself with AI?”", paras: [
          ...
          { heading: "“Do you only do greenfield, solo builds?”", paras: [
          ...
          { heading: "“Will you maintain it after launch?”", paras: [
      ...
      "off-clock": {
        label: "Off the clock", type: "page", desc: "The human running the agents",
        sections: [
          { paras: [
            "The rest of this portfolio is the work. This page is the person running it.",
          ]},
          { heading: "Where I actually am", paras: [
      ...
      "go-paradox": { label: "Paradox", type: "ws", ws: "paradox", desc: "A living multiverse of stories" },
      "go-shiftsum": { label: "Shiftsum", type: "ws", ws: "shiftsum", desc: "Shift scheduling for hourly teams" },
      "go-clients": { label: "Client builds", type: "ws", ws: "clients", desc: "Selected engagements, 2–12 weeks" },
      "component-library": LIBRARY,
    },
  },
  {
    id: "paradox", initials: "P", name: "Paradox", desc: "A living multiverse of stories",
    status: "concept · 2026", accent: "#3edba6",
    ...
    items: {
      overview: {
        label: "Overview", type: "page", desc: "A storytelling app where the timeline is the interface",
        hero: { frag: "multiverse", kicker: "the homepage", lead: "No feed. No thumbnail grid. You land inside a living graph of stories and traverse it — drag, zoom, click a node, enter a reality." },
        sections: [
          { paras: [
      ...
      "living-timeline": {
        label: "The living timeline", type: "comp", frag: "multiverse", desc: "The homepage is a place, not a page",
        paras: [
      ...
      "gateway-comp": {
        label: "Sign-in gateway", type: "comp", frag: "gateway", desc: "Guest-first, passwords never",
        paras: [
      ...
      "palette-comp": {
        label: "Command palette", type: "comp", frag: "palette", desc: "The only menu in the product",
        paras: [
  ...
  {
    id: "shiftsum", initials: "S", name: "Shiftsum", desc: "Shift scheduling for hourly teams",
    status: "acquired 2023", accent: "#e3a84e",
    ...
    items: {
      overview: {
        label: "Overview", type: "page", desc: "Built, grown, acquired",
        hero: { frag: "admin", kicker: "the screen managers lived in", lead: "Search, filter, bulk-approve, zero page reloads. The roster screen that took ~20 minutes a day down to ~4." },
        sections: [
          { paras: [
      ...
      onboarding: {
        label: "Onboarding", type: "comp", frag: "onboarding", desc: "From 9 fields to 3 questions", unread: true,
        paras: [
      ...
      notifications: {
        label: "Notifications", type: "comp", frag: "notifications", desc: "Quiet by default",
        paras: [
      ...
      admin: {
        label: "Roster admin", type: "comp", frag: "admin", desc: "The screen managers lived in",
        paras: [
  ...
  {
    id: "clients", initials: "CB", name: "Client builds", desc: "Selected engagements, 2–12 weeks",
    status: "3 of 9 shown", accent: "#a98bf5",
    ...
    items: {
      overview: {
        label: "Overview", type: "page", desc: "Other people's products, same standards",
        hero: { frag: "status", kicker: "a public status page", lead: "A two-day build. The week it shipped the client's API had a bad day — and support tickets halved, because customers could watch the fix happen." },
        sections: [
          { paras: [
      ...
      "booking-widget": {
        label: "Booking widget", type: "comp", frag: "booking", desc: "A physio clinic, off the phone",
        paras: [
      ...
      "kyc-flow": {
        label: "Identity verification", type: "comp", frag: "kyc", desc: "The code I didn't write", unread: true,
        paras: [
      ...
      "status-page": {
        label: "Status page", type: "comp", frag: "status", desc: "Two days, lasting trust",
        paras: [
...
export const LIBRARY_INTRO = "Every piece from every project, in one feed. Filter by what your product needs — odds are I've already built a version of it.";
